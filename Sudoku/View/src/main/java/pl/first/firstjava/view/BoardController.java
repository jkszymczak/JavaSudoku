...
public class BoardController implements Initializable {
    ...
    // fx:id's for our buttons? name should be the same in order for the FXMLLoader
    // to find it.
    @FXML
    Button buttonTwo;
    @FXML
    ...
    public void drawOnCanvas(GraphicsContext context) throws CloneNotSupportedException {
        ...
        for (int row = 0; row < 9; row++) {
            for (int col = 0; col < 9; col++) {
                // finds the y position of the cell, by
                // multiplying the row number by 50, which is the height of a row
                // then adds 2, to add some offset
                int positionY = row * 50 + 2;

                // finds the x position of the cell, by multiplying the column
                // number by 50, which is the width of a
                // then add 2, to add some offset
                int positionX = col * 50 + 2;

                // defines the width of the square as 46 instead of 50, to account for the
                // 4px total of blank space
                ...
                // rounded corner arcs width and height.
        ...
        // draw a strokeRoundRect using the same technique we used for drawing our
        // board.
        ...
        for (int row = 0; row < 9; row++) {
            for (int col = 0; col < 9; col++) {
                ...
                // check if value of coressponding initial array position is not 0, remember
                // that
     ...
     * for(int col = 0; col<9; col++) {
     * 
     * // finds the y position of the cell, by multiplying the row number by 50,
     * which
     * // is the height of a row in pixels then adds 2, to add some offset
     ...
     * context.setFill(Color.BLACK);
     * 
     * // set the font, from a new font, constructed from the system one, with size
     * 20
     * context.setFont(new Font(20));
     * 
     * // check if value of coressponding initial array position is not 0, remember
